...
rf_reg =RandomForestRegressor(n_estimators=10 ,random_state=0) #ikincisi kaç tane tree çizileceğini söyler
rf_reg.fit(X, Y)

plt.scatter(X, Y, color='red')
plt.plot(X, svr_reg.predict(X), color='blue') #tahmin değerinin önce polynomial yap
plt.show()

print(rf_reg.predict(np.array([[6.6]])))
